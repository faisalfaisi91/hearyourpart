...
jq(document).ready( function() {
	...
	jq('#aw-whats-new-submit').on( 'click', function() {
		...
		firstrow = jq( '#youzer ul.activity-list li' ).first();
	...
	jq('div.activity').on( 'click', function(event) {
		...
		if ( target.parent().hasClass('load-more') ) {
			...
			jq('#youzer li.load-more').addClass('loading');
			...
			function(response)
			{
				jq('#youzer li.load-more').removeClass('loading');
				activity_oldestpage = oldest_page;
				var new_activities = jq( response.contents );
				jq('#youzer ul.activity-list').append( new_activities );
				target.parent().hide();
		...
		if ( target.parent().hasClass('load-newest') ) {
			...
			jq( '#youzer ul.activity-list' ).prepend( newest_activities );
	...
	jq('#youzer').on( 'click', function(event) {
		var target = jq(event.target),
			el,
	...
	jq('#youzer').on('click', '.group-button .leave-group', function() {
	...
	jq('#youzer').on( 'click', '.pending', function() {
		return false;
	});
	...
	jq( document ).on( 'heartbeat-send.buddypress', function( e, data ) {
		...
		if ( jq( '#youzer ul.activity-list li' ).first().prop( 'id' ) ) {
			// getting the timestamp
			timestamp = jq( '#youzer ul.activity-list li' ).first().prop( 'class' ).match( /date-recorded-([0-9]+)/ );
	...
	jq( document ).on( 'heartbeat-tick', function( e, data ) {
		...
		if ( jq( '#youzer ul.activity-list li' ).first().hasClass( 'load-newest' ) ) {
			return;
		}

		jq( '#youzer ul.activity-list' ).prepend( '<li class="load-newest"><a href="#newest">' + BP_DTheme.newest + '</a></li>' );
	});
});
		...
		if ( caller === 'pag-bottom' && jq('#subnav').length ) {
			...
			jq('html,body').animate({scrollTop: top.offset().top}, 'slow', function() {
				jq(target).fadeOut( 100, function() {
					jq(this).html(response);
					jq(this).fadeIn(100);
					if ( jq( '#yz-members-directory' )[0] && jq( '#yz-members-list' )[0] ) {
					...
					if ( jq( '#yz-groups-directory' )[0] && jq( '#yz-groups-list' )[0] ) {
...
function bp_legacy_theme_hide_comments( container ) {
	...
	var comments_lengths = window.hasOwnProperty( 'yz_comments_length' ) ? window.yz_comments_length : 5;
