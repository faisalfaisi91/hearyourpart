( function( $ ) {
	...
	$( document ).ready( function() {
		...
		$( document ).on( 'click', '.yz-wall-post', function() {
			...
			/* Default POST values */
			var object = '';
			var item_id = form.find( '#yz-whats-new-post-in' ).val();
			...
			if ( item_id > 0 ) {
				object = form.find( '#yz-whats-new-post-object' ).val();
			}
			...
			$.post( ajaxurl, post_data, function( response ) {
				...
				if ( $.yz_isJSON( response ) ) {
		            ...
		            form.find( '#yz-whats-new-post-in' ).find( 'option' ).first().trigger( 'click' );

		            // Update Nice Select.
		            if ( ! window.hasOwnProperty( 'yz_disable_niceselect' ) ) {
		            	form.find( '#yz-whats-new-post-in' ).niceSelect( 'update' );
		            }
