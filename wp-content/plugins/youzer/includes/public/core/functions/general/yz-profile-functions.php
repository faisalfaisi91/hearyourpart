...
function yz_get_open_graph_tags( $type = null, $url = null,  $title = nul, $description = null, $image = null, $size = null ) {
    ...
    <?php if ( ! empty( $image ) ) : ?>
        <?php if ( empty( $size ) ) $size = yz_get_image_size( $image ); ?>
        <meta property="og:image" content="<?php echo $image; ?>">
        <meta property="og:image:url" content="<?php echo $image; ?>">
        <?php if ( ! empty( $size ) ) : ?>
            <meta property="og:image:width" content="<?php echo isset( $size['width'] ) ? $size['width'] : $size[0]; ?>">
            <meta property="og:image:height" content="<?php echo isset( $size['height'] ) ? $size['height'] : $size[1]; ?>">
        <?php endif; ?>
    <?php endif; ?>
...
function yz_get_post_thumbnail( $args = false ) {
    ...
    if ( 'post' == $widget ) {
        ...
        $img_size = isset( $args['img_size'] ) ? $args['img_size'] : apply_filters( 'yz_default_blog_post_image_size','medium' );
        ...
        if ( $post_id ) {
            ...
            if ( ! empty( $img_url[0] ) ) {
                echo '<div class="yz-post-thumbnail"><img loading="lazy" ' . yz_get_image_attributes(apply_filters( 'yz_blog_posts_thumbnail', $img_url[0] ) ) . ' alt=""></div>';
            } else {
                // Get Post Format
        ...
        if ( $img_url ) {
            echo "<div class='yz-$widget-thumbnail'><img loading='lazy' " . yz_get_image_attributes( $img_url ) . " alt=''></div>";
        } else {
            echo '<div class="yz-no-thumbnail">';
