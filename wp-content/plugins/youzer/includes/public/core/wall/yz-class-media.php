...
class Youzer_Media {
    ...
    function get_photos_items( $args = null ) {
        ...
        foreach ( $photos as $photo ) : ?>
        	<?php $src = maybe_unserialize( $photo['src'] ); $img_url = yz_get_media_url( $src ); ?>
            <div data-item-id="<?php echo $photo['id']; ?>" class="yz-media-item"><div class="yz-media-item-img" ><img loading="lazy" <?php echo yz_get_image_attributes( $img_url ); ?> alt=""><div class="yz-media-item-tools"><a href="<?php echo bp_activity_get_permalink( $photo['item_id'] );?>"><i class="fas fa-link yz-media-post-link"></i></a><a data-lightbox="yz-media-lightbox" href="<?php echo $img_url; ?>"><i class="fas fa-search yz-media-zoom-photo"></i></a></div></div></div>
        <?php endforeach;
        ...
        foreach ( $videos as $video ) :
        	...
        	<?php if ( $thumbnail_type == 'image' ) : ?>
        		<div data-activity-id="<?php echo $video['item_id']; ?>" class="yz-media-item-content">
                    <div class="yz-media-item-img yz-<?php echo $src['provider']; ?>-item"><img loading="lazy" <?php echo yz_get_image_attributes( $thumbnail ); ?> alt=""><?php if ( $src['provider'] == 'local' ) echo '<i class="fas fa-play-circle yz-media-local-video"></i>'; ?><div class="yz-media-item-tools"><a href="<?php echo bp_activity_get_permalink( $video['item_id'] );?>"><i class="fas fa-link yz-media-post-link"></i></a><a class="yz-video-lightbox" data-<?php echo $this->get_video_data( $src['provider'] ); ?>="<?php echo $src['original']; ?>"><i class="fas fa-play-circle yz-media-video-play"></i></a></div></div></div>
        	<?php else : ?>
            	<video width="100%" controls preload="metadata"><source class="lazyload" src="<?php echo yz_get_media_url( $src ); ?>" type="video/mp4"><?php echo __( 'Your browser does not support the video tag.', 'youzer' ); ?></video>
        	<?php endif; ?>
        	</div>
