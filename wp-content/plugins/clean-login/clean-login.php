...
function clean_login_edit_show( $atts ) {
	...
	if ( is_user_logged_in() ) {
		clean_login_get_template_file( 'login-edit.php', $atts );
	} else {
...
function clean_login_load_before_headers() {
	...
	//check if clean_login_wpnonce exists as post parameter
	if( isset( $_POST["clean_login_wpnonce"] ) ){
	    $nonce = isset( $_POST["clean_login_wpnonce"] ) ? $_POST["clean_login_wpnonce"] : "";
	    $cleanlogin_has_verified_nonce = wp_verify_nonce( $nonce, 'clean_login_wpnonce' );
	}
 ...
 */
function clean_login_get_pages_with_shortcodes( $post_id, $post ) {
...
add_action( 'save_post', 'clean_login_get_pages_with_shortcodes', 10, 2 );
