...
class AjaxyLiveSearch {
	...
	function enqueue_styles() {
		...
		$style = AJAXY_SF_PLUGIN_URL."themes/dark/style.css";
	...
	function head()
	{
		...
		$settings = array(
			'label' => $this->get_style_setting('search_label', ''),
	...
	function admin_notice()
	{
		...
		/* if(!class_exists ( 'AjaxyTracker' ) && (($dismiss != 1 && $dismiss != 2) || ($current_screen->parent_base == 'ajaxy-page' && $dismiss != 2))) {
			 echo '<div class="updated"><p><b>Theme20:</b> ssssssssssss | <a href="'.(strpos( $_SERVER['REQUEST_URI'], '?') !== false ? $_SERVER['REQUEST_URI'].'&ajaxy-tdismiss=1' : $_SERVER['REQUEST_URI'].'?ajaxy-tdismiss=1').'">X</a></p></div>';
		} */
	}
...
<form method="get" class="searchform" action="' . home_url( '/' ) . '">
	<input class="sf_input" type="text" value="' . get_search_query() . '" name="s" id="s" />
