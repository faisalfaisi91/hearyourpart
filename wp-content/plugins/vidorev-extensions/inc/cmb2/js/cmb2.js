...
(function(window, document, $, cmb, undefined){
	...
	cmb.addGroupRow = function( evt ) {
		...
		/*VidoRev Custom*/
		if(typeof(window.CMBAP) !== 'undefined' && typeof(window.CMBAP.init) === 'function' && $(evt.currentTarget).attr('data-selector') == 'video_series_group_repeat'){
			$newRow.find('.attached-wrap.column-wrap ul.attached.connected').html('');
			$newRow.find('.retrieved-wrap.column-wrap ul.retrieved.connected > li').removeClass('added');
			window.CMBAP.init();			
		}
		
		if($(evt.currentTarget).attr('data-selector') == 'vm_video_multi_links_repeat' && $('#multiple_links_structure').length > 0){
			$('#multiple_links_structure').trigger('change');
		}
		
		if($(evt.currentTarget).attr('data-selector') == 'filter_tags_settings_group_repeat'){
			
			var $old_tags_field = $('#'+(getRowId( $oldRow.attr('id') ))).find('.custom-filter-tags-display-control');
			
			if($old_tags_field.length > 0){						
				$old_tags_field.html('');
			}
			
			if(typeof($.fn.select2)!=='undefined' && $old_tags_field.length > 0){
				...
				$old_tags_field.html('<select id="filter_tags_settings_group_'+(cmb.idNumber)+'_tags" data-placeholder="..." class="vidorev-admin-ajax admin-ajax-find-tags-control" name="filter_tags_settings_group['+(cmb.idNumber)+'][tags][]" multiple></select>');
				
				$('#filter_tags_settings_group_'+(cmb.idNumber)+'_tags').select2({
					ajax: 	{
				...
				$old_tags_field.find('ul.select2-selection__rendered').sortable({
				  containment: 'parent',
				});	
			}			
