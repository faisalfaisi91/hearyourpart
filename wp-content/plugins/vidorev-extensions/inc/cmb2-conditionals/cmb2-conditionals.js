jQuery( document ).ready( function( $ ) {
	...
	/**
	 * Add 'show-cmb' and 'hide-cmb' event to JQuery event detection.
	 * @see http://viralpatel.net/blogs/jquery-trigger-custom-event-show-hide-element/
	 */
	$.each( ['show-cmb', 'hide-cmb'], function( i, ev ) {
		var el = $.fn[ev];
		$.fn[ev] = function() {
			this.trigger( ev );
	...
	function CMB2ConditionalsInit( changeContext, conditionContext ) {
		...
		conditionContext.on( 'hide-cmb', '.cmb-row', function() {
		...
		conditionContext.on( 'show-cmb', '.cmb-row', function() {
			$( this ).children( '[data-conditional-required="required"]' ).each( function( i, e ) {
